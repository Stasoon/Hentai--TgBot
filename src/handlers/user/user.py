...
from src.database.hentai import get_hentai_by_code_or_none, get_random_hentai_code
...
from src.misc import hentai_callback, favorite_nav_callback, HentaiSearching, SubscriptionChecking
from .messages import Messages
from .kb import Keyboards


async def send_hentai(code: int, user_id: int) -> bool:
    hentai_data = get_hentai_by_code_or_none(code)
    ...
    await bot.send_message(chat_id=user_id, text=Messages.get_hentai_not_found(code),
...
async def send_hentai_without_photo(title, descript, code, user_id):
    await bot.send_message(
        chat_id=user_id,
        text=Messages.get_formatted_hentai_description(title=title, description=descript),
        reply_markup=Keyboards.get_hentai_markup(user_id, hentai_code=code)
    )
...
async def send_hentai_with_photo(title, descript, photo, code, user_id):
    await bot.send_photo(
        chat_id=user_id,
        caption=Messages.get_formatted_hentai_description(title=title, description=descript),
        photo=photo,
        reply_markup=Keyboards.get_hentai_markup(user_id, hentai_code=code)
    )
...
# !!! ПРОВЕРКА ПОДПИСКИ !!! #
@throttle()
async def __handle_search_button(message: types.Message, state: FSMContext):
    await send_typing_action(message)

    await message.answer(Messages.get_search(), reply_markup=Keyboards.get_back_markup())
    await state.set_state(HentaiSearching.wait_for_code)
...
async def __handle_random_hentai_button(message: types.Message):
    ...
    hentai_random_code = get_random_hentai_code()
...
async def __handle_show_favorite_hentai_callback(callback: types.CallbackQuery, callback_data: hentai_callback):
...
async def __handle_hentai_favorite_callback(callback: types.CallbackQuery, callback_data: hentai_callback) -> None:
    code = callback_data.get('code')
    user_id = callback.from_user.id

    add_or_delete_favorite(user_id, code)
    new_markup = Keyboards.get_hentai_markup(user_id, code)
    try:
...
def register_user_handlers(dp: Dispatcher) -> None:
    ...
    # обработка поиска хентая
    dp.register_message_handler(__handle_search_button, text='🔎 Искать по коду', state=None)
    dp.register_message_handler(__handle_code_message, content_types=['text'], state=HentaiSearching.wait_for_code)
    dp.register_callback_query_handler(__handle_back_callback, text='back', state=HentaiSearching.wait_for_code)

    # обработка нажатия на рандом
    dp.register_message_handler(__handle_random_hentai_button, text='🎲 Случайный хентай')
    ...
    dp.register_callback_query_handler(__handle_show_favorite_hentai_callback, hentai_callback.filter(action='show'))
    # удаление / добавление в избранное
    dp.register_callback_query_handler(__handle_hentai_favorite_callback, hentai_callback.filter(action='favorite'))
